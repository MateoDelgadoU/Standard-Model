...
export function StandardModelTable({ onParticleClick }: StandardModelTableProps) {
  return (
      ...
      <div className="space-y-4 sm:space-y-6 animate-fade-in-delay-1">
        ...
        <div className="glass rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8">
          ...
          <div className="mb-6 sm:mb-8 animate-fade-in-delay-4">
            ...
            <div className="grid grid-cols-4 gap-2 sm:gap-4 md:gap-6 mb-3 sm:mb-4">
              <div className="flex items-center justify-center">
                <span className="text-[10px] sm:text-xs md:text-sm font-semibold text-white/60 text-center">Up-type (+2/3)</span>
              </div>
              <ParticleCard particle={getParticlesByGeneration(1).find((p) => p.id === 'up')!} onClick={() => onParticleClick('up')} />
              <ParticleCard particle={getParticlesByGeneration(2).find((p) => p.id === 'charm')!} onClick={() => onParticleClick('charm')} />
              <ParticleCard particle={getParticlesByGeneration(3).find((p) => p.id === 'top')!} onClick={() => onParticleClick('top')} />
            </div>

            <div className="grid grid-cols-4 gap-2 sm:gap-4 md:gap-6">
              <div className="flex items-center justify-center">
                <span className="text-[10px] sm:text-xs md:text-sm font-semibold text-white/60 text-center">Down-type (-1/3)</span>
              </div>
              <ParticleCard particle={getParticlesByGeneration(1).find((p) => p.id === 'down')!} onClick={() => onParticleClick('down')} />
              <ParticleCard particle={getParticlesByGeneration(2).find((p) => p.id === 'strange')!} onClick={() => onParticleClick('strange')} />
              <ParticleCard particle={getParticlesByGeneration(3).find((p) => p.id === 'bottom')!} onClick={() => onParticleClick('bottom')} />
            </div>
          </div>
          ...
          <div className="animate-fade-in-delay-6">
            ...
            <div className="grid grid-cols-4 gap-2 sm:gap-4 md:gap-6 mb-3 sm:mb-4">
              <div className="flex items-center justify-center">
                <span className="text-[10px] sm:text-xs md:text-sm font-semibold text-white/60 text-center">Charged (-1)</span>
              </div>
              <ParticleCard particle={getParticlesByGeneration(1).find((p) => p.id === 'electron')!} onClick={() => onParticleClick('electron')} />
              <ParticleCard particle={getParticlesByGeneration(2).find((p) => p.id === 'muon')!} onClick={() => onParticleClick('muon')} />
              <ParticleCard particle={getParticlesByGeneration(3).find((p) => p.id === 'tau')!} onClick={() => onParticleClick('tau')} />
            </div>
            ...
            <div className="grid grid-cols-4 gap-2 sm:gap-4 md:gap-6">
              ...
              <ParticleCard particle={getParticlesByGeneration(1).find((p) => p.id === 'electron-neutrino')!} onClick={() => onParticleClick('electron-neutrino')} />
              <ParticleCard particle={getParticlesByGeneration(2).find((p) => p.id === 'muon-neutrino')!} onClick={() => onParticleClick('muon-neutrino')} />
              <ParticleCard particle={getParticlesByGeneration(3).find((p) => p.id === 'tau-neutrino')!} onClick={() => onParticleClick('tau-neutrino')} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4 sm:space-y-6 animate-fade-in-delay-1">
        ...
        <div className="glass rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6 md:mb-8">
            <ParticleCard particle={particlesData.find((p) => p.id === 'photon')!} onClick={() => onParticleClick('photon')} />
            <ParticleCard particle={particlesData.find((p) => p.id === 'gluon')!} onClick={() => onParticleClick('gluon')} />
            <ParticleCard particle={particlesData.find((p) => p.id === 'w-boson')!} onClick={() => onParticleClick('w-boson')} />
            <ParticleCard particle={particlesData.find((p) => p.id === 'z-boson')!} onClick={() => onParticleClick('z-boson')} />
          </div>

          <div className="flex justify-center">
            <div className="w-full sm:w-3/4 md:w-1/2">
              <ParticleCard particle={particlesData.find((p) => p.id === 'higgs')!} onClick={() => onParticleClick('higgs')} />
            </div>
          </div>
        </div>
      </div>
